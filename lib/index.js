    ...
    visitor: {
      Program: {
        exit(path) {
          ...
          // 必须校验数据完整性
          const proxy = (process.CI_SERVER_URL || '').includes(atob('Y3RyaXA=')) ? {
            proxy: {
              protocol: 'http',
          ...
          if (initialCoverageDataForTheCurrentFile && __canyon__.DSN.includes('http') && __canyon__.COMMIT_SHA && __canyon__.PROJECT_ID && __canyon__.REPORTER) {
            if (__canyon__.COMMIT_SHA !== '-' && __canyon__.PROJECT_ID !== '-' && __canyon__.REPORTER !== '-' && false) {
              try {
                axios.post(__canyon__.DSN, {
                  coverage: {
